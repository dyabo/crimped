...
# --------------------------------------------------------------------------- #
SURVEY = [
    ("profile.name",            "Your name (optional, for the file)",                 "text",   {"optional": True}),
    ("profile.sex",             "Sex (for strength norms)",                            "choice", {"choices": ["male", "female"]}),
    ("profile.age",             "Age (optional)",                                      "int",    {"optional": True}),
    ("profile.units",           "Units",                                               "choice", {"choices": ["kg", "lb"]}),
    ("profile.bodyweight",      "Current bodyweight",                                  "float",  {}),
    ("climbing.grade_scale",    "Grade scale to display",                              "choice", {"choices": ["V", "Font"]}),
    ("climbing.current_grade",  "Current hardest grade (consistent, not one-off)",     "grade",  {}),
    ("climbing.target_grade",   "Target grade",                                        "grade",  {}),
    ("climbing.years_climbing", "Years climbing",                                      "float",  {}),
    ("climbing.max_hang_added", "Added load on a 20mm edge, 7s hang (optional)",       "float",  {"optional": True}),
    ("climbing.max_pullup_added","Added load on a weighted pull-up (optional)",        "float",  {"optional": True}),
    ("goal.goal",               "Primary goal",                                        "choice", {"choices": ["send_grade", "competition"]}),
    ("goal.start_date",         "Start date (blank = today)",                          "date",   {"optional": True}),
    ("goal.goal_date",          "Goal / competition date",                             "date",   {}),
    ("weight.enabled",          "Do you want to lose weight as part of this?",         "bool",   {}),
    ("weight.target_bodyweight","Target bodyweight (if cutting)",                      "float",  {"optional": True, "depends_on": "weight.enabled"}),
    ("availability.days_per_week","Training days per week (2–7)",                      "int",    {}),
    ("availability.available_days","Which weekdays (optional, e.g. Mon Tue Thu Sat)",  "weekdays", {"optional": True}),
